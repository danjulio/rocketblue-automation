...
I encourage you to read the Pi Platter Manual found in the [documentation](https://github.com/danjulio/rocketblue-automation/tree/master/pi_platter/documentation) directory as well as the thorough documenation in the [motioneyeos wiki](https://github.com/ccrisan/motioneyeos/wiki).
...
5. 2000-6000 mA 3.7 volt LiPo battery.  I used a [2000 mA battery from Sparkfun](https://www.sparkfun.com/products/13855) that should provide at least 2 hours of run-time.
...
## Hardware assembly
Assembly is straight-forward.  The Pi Zero mounts on top of the Pi Platter using the included mounting hardware.  This assembly mounts in the enclosure in a stacked arrangment.
...
The motioneyeos distribution creates a partion, ```/data```, that is used to store persistent changes.  It is created during the first boot.  We manually put the additional files in this partition (```/data/etc```).
...
2. The five scripts found in the ```scripts``` directory here use the motioneyeos [Action Button](https://github.com/ccrisan/motioneyeos/wiki/Action-Buttons) capabilty to allow control of the servos and for displaying information from the Pi Platter in the web interface.
...
Download the files to your local computer and execute the following commands from the directory containing the files.  Put your Pi's IP address in the ```[IP Address]``` field.
...
2. Eject the memory stick and then plug it into one of the Pi Platter USB ports.  Motioneyeos will mount it in the ```/data/media``` directory.
